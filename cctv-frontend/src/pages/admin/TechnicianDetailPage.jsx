...
const TechnicianDetailPage = () => {
  ...
  const { data: technicianDetails, isLoading } = useQuery({
    queryKey: ['technician', id],
    queryFn: async () => {
      const { data } = await adminApi.getTechnician(id);
      return data;
    },
    enabled: !!id,
  });
  ...
  const updateMutation = useMutation({
    mutationFn: (data) => adminApi.updateTechnician(id, data),
    onSuccess: () => {
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center h-64">
        <Loading message="Loading technician details..." />
      </div>
    );
  }
  ...
  if (!technicianDetails) {
    return (
      <div className="p-8 text-center bg-white border border-slate-200 rounded-2xl">
        <p className="text-slate-500 font-semibold mb-4">Technician not found.</p>
        <Button onClick={() => navigate('/admin/technicians')} className="flex items-center space-x-2 mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-1">
          <GlassCard className="p-6 border border-slate-200/60 shadow-xl shadow-slate-200/20 bg-white/70">
            <h2 className="text-lg font-bold border-b border-slate-100 pb-2.5 mb-4 text-slate-800">Technician Details</h2>
            <div className="space-y-4">
              <div>
                <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Full Name</span>
                <span className="text-sm font-semibold text-slate-800 block mt-0.5">{technicianDetails.name}</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Email Address</span>
                <span className="text-sm font-semibold text-slate-800 block mt-0.5">{technicianDetails.email}</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Phone Number</span>
                <span className="text-sm font-semibold text-slate-800 block mt-0.5">{technicianDetails.phone}</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Skills</span>
                <span className="text-xs text-slate-700 block mt-1 leading-relaxed bg-slate-100 p-2 rounded border border-slate-200/60 font-medium">
                  {technicianDetails.skills || 'No skills listed'}
                </span>
              </div>
              <div className="pt-2">
                ...
                <button 
                  ...
                  className={`px-3 py-1 rounded-full text-xs font-semibold transition-all shadow-sm ${
                    technicianDetails.is_active 
                      ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200 border border-emerald-200' 
                      : 'bg-rose-100 text-rose-700 hover:bg-rose-200 border border-rose-200'
                  }`}
                  title="Click to toggle status"
                >
                  {technicianDetails.is_active ? 'Active' : 'Inactive'}
                </button>
              </div>
            </div>
          </GlassCard>
          ...
          <GlassCard className="p-6 border border-slate-200/60 shadow-xl shadow-slate-200/20 bg-white">
            ...
            <div className="flex border-b border-slate-200 mb-6">
              ...
              >
                ...
                <span>Assigned Tickets ({technicianDetails.tickets?.length || 0})</span>
              </button>
              <button 
            ...
            <div className="min-h-[300px]">
              ...
              {activeTab === 'edit' && (
                <form onSubmit={handleEditSubmit} className="space-y-4 max-w-xl">
                  ...
                  <Input label="Skills (comma separated)" name="skills" value={editFormData.skills} onChange={handleEditInputChange} placeholder="e.g. Cameras, DVRs, Networking" required />
                  ...
                  <div className="flex justify-end gap-3 pt-6 border-t border-slate-100 mt-6">
                    <Button variant="secondary" onClick={() => navigate('/admin/technicians')} type="button">Cancel</Button>
